const Footer = () => {
  return (
    <section className="px-[2rem] notebook:px-[8rem] py-[1.5rem] notebook:py-[2rem] bg-black flex flex-col notebook:flex-row justify-between items-center notebook:items-end">
      <div className="text-[.7rem] notebook:text-[1rem] p-[1rem] text-white font-bold border-[#00F000] border-b-2 mb-[1rem]">
    
        Einax LLC
        <br />
        1504 Bay Road
        <br />
        Miami Beach Fl, 33139
      </div>
      <div className="text-[.6rem] notebook:text-[1rem] text-white font-bold ">
      Einax © 2023. All Rights Reserved.
      </div>
    </section>
  );
};

export default Footer;
